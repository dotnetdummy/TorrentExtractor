...
namespace TorrentExtractor.Settings
{
    public class Paths
    {
        ...
        /// <summary>
        /// Comma separated list of words to whitelist files in source directory. If not set, all files will be processed.
        /// </summary>
        ...
        /// <summary>
        /// Array of words to whitelist files in source directory. If empty, all files will be processed.
        /// </summary>
        public string[] WhitelistedWordsAsArray => WhitelistedWords?.Split(',').ToArray() ?? Array.Empty<string>();
        
        /// <summary>
        /// Array of words to blacklist files in source directory. If empty, all files will be processed.
        /// </summary>
        public string[] BlacklistedWordsAsArray => BlacklistedWords?.Split(',').ToArray() ?? Array.Empty<string>();
        ...
        public void Validate()
        {
            ...
            if(string.IsNullOrWhiteSpace(Movies?.Default))
                throw new ValidationException($"A valid {nameof(Movies)}__{nameof(Movies.Default)} is required!");
            
            if(string.IsNullOrWhiteSpace(Tv?.Default))
                throw new ValidationException($"A valid {nameof(Tv)}__{nameof(Tv.Default)} is required!");
        }
